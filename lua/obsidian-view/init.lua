...
M.config = {
    ...
    height = 0.8,
    border = "rounded",
    -- Folders to exclude from view
    exclude_folders = {},
    -- If set, only show notes from these folders
    include_folders = {},
    -- Number of preview lines for description
    preview_lines = 3
}

-- Function to get obsidian.nvim instance
local function get_obsidian()
    local ok, obsidian = pcall(require, "obsidian")
    if not ok then
        error("obsidian.nvim is required but not found")
        return nil
    end
    return obsidian
end
...
local function get_notes()
    ...
    local function should_include(path)
        ...
        local rel_path = path:sub(#vault_path + 2)
